...
def main():
    ...
    st.markdown(
        ...
        <style>
            .stApp {
                background: linear-gradient(to top, rgb(235, 196, 241), rgb(248, 215, 208));
            }
        </style>
        """,
    ...
    if password and check_button:
        ...
        st.markdown(
            f"""
            <p style='color:white; background-color:{color}; padding:10px; border-radius:5px; text-align:center;'>
            Password Strength: <b>{strength_label}</b></p>
            """, 
            unsafe_allow_html=True
        )
